...
export function VideoPlayer({ streamUrl, isLive = false, onClose }: VideoPlayerProps) {
  ...
  // Smart TV Hotkeys: Enter (Play/Pause), M (Mute), Arrows (Seek & Volume)
  useEffect(() => {
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key.toLowerCase() === 'm') {
        ...
        setIsMuted(newMute)
        if (!newMute && videoRef.current.volume === 0) setVolume(1)
        setShowControls(true);
        return;
      }

      // Volume Controls (TV Style)
      if (e.key === 'ArrowUp') {
        e.preventDefault();
  ...
  useEffect(() => {
    ...
    // 🔙 Interceptor Global Rider TV (Sustituye a setupBackListener local)
    const cleanupInterceptor = navigationBus.registerInterceptor(() => {
      console.log('⚡ Player: Interceptando botón atrás');
      if (isNativePlatform) {
      ...
      return false; // Propagar a behavior por defecto
    });
