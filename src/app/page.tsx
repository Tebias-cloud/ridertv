  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const { error, data } = await supabase.auth.signInWithPassword({
        email: `${username}@rider.com`,
        password,
      })

      if (error) {
        setErrorMsg(error.message.includes('Invalid') ? 'Credenciales incorrectas' : 'Error del servidor')
    ...
    <div className="relative flex flex-col items-center justify-center min-h-[100dvh] bg-[#0c0c1e] px-8 overflow-hidden font-sans">
      ...
      <div className="relative z-10 w-full max-w-sm space-y-12">
        <div className="text-center">
          ...
          <p className="mt-4 text-[#a0a0b8] font-medium text-[15px] opacity-80 tracking-tight">
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          <div className="space-y-4">
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute inset-y-0 right-0 w-16 flex items-center justify-center text-[#5c5c7a] hover:text-white transition-colors"
              >
          ...
          {errorMsg && (
            <div className="bg-red-600/5 border border-red-600/20 py-3 px-4 rounded-xl text-center">
              <p className="text-sm font-bold text-red-500 italic">{errorMsg}</p>
            </div>
          )}
          ...
          <div className="pt-4">
            ...
            <div className="mt-14 text-center">
              <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="text-[13px] text-[#a0a0b8] hover:text-white font-medium transition-colors">
